...
contract SyfinNFTSale {

    address admin;
    SyfinNFT public syfinNFT;

    address public immutable SYF = address(0x1BCF4DC879979C68fA255f731FE8Dcf71970c9bC); // SYF
    ...
    function BuyNFT(address _owner, uint256 _tokenId, uint256 _price) public payable {
        require(syfinNFT.ownerOf(_tokenId) == _owner);
        uint price;
        (, , , , , , price) = syfinNFT.imageData(_tokenId);
        require(_price == price);

        IERC20(SYF).transferFrom(msg.sender, _owner, _price*1e18);
        syfinNFT.transferFrom(_owner, msg.sender, _tokenId);
        syfinNFT.nftSold(_tokenId);
    ...
    function GiftNFT(address _gifter, uint256 _tokenId, address _receiver) public payable {
        ...
        require(syfinNFT.ownerOf(_tokenId) == msg.sender);
        require(msg.sender == _gifter, "Only owner can gift NFT");

        syfinNFT.transferFrom(_gifter, _receiver, _tokenId);
        syfinNFT.nftSold(_tokenId);
