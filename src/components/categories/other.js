    ...
    async function getNFTs() {
        ...
        if (networkData) {
            ...
            fetch('https://nft.sy.finance/api/nftlist.json', { mode: 'cors'}).then(response => {
                ...
                const result = fuse.search("=Other$");
                if (!result.length) {
                    setData([]);
                    // setData(images);
                } else {
        ...
        return (
            <div>
                <div className="head-title col-auto mx-4">
                    <h4 className="mb-0 font-weight-normal">All <strong>{images.length}</strong> "Other" NFTs on SYF Fantom</h4>
                </div>
                <div className="container-fluid mb-5 explore-adj">
                    <div className="row justify-content-around">
                    <p align="center" className="text-secondary">This is where you can explore all "Other" category NFTs that were minted within SYF NFT Market.</p>
                    {/* <Progress
                        type="circle"
                        ...
                        /> */}
                    <p><div align="center" style={{ maxWidth: "500px", margin: "0 auto"}}><input placeholder="Search all 'Other' SYF NFTs..." type="text" className="form-control my-2" onChange={(e) => searchData(e.target.value)} disabled={loaded}></input></div></p>
                    {/* <div align="center">
                        <ReactPaginate
                            pageCount={pageCount}
